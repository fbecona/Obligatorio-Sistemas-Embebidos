...
/** 
  @Function
  void anilloColor(ws2812_t c_led)

  @Summary
  Enciende o apaga el anillo
  @Parameters
  c_led:El color con el que se setean los LEDS
 */
void anilloColor(ws2812_t c_led){
    ws2812_t ledArray[WS2812_LED_QTY];
    uint8_t i;
    for(i = 0; i < WS2812_LED_QTY; i++) {
        ledArray[i] = c_led;      
    }
    WS2812_send(ledArray, WS2812_LED_QTY);
    i++;
}
