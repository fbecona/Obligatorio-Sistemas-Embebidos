    ...
    while( 1 ){
        if( xSemaphoreTake( c_semIsModuleOnAndReady, pdMS_TO_TICKS( SIM808_TURN_ON_GSM_TIMEOUT_ms ) )==pdTRUE ){
            if( (resultExchange = SIM808_exchangeCmd( ATREQ_PIN_QUERY, ATRES_PIN_READY, SIM808_UART_TIMEOUT_ms, SIM808_MAX_RETRIES ))==true ){
                if( (resultExchange = SIM808_exchangeCmd( ATREQ_OP_MOVISTAR, ATRES_OK, SIM808_UART_TIMEOUT_ms, SIM808_MAX_RETRIES ))==true ){
...
/**
 * @Function
 *  int8_t SIM808_receiveSMS(uint8_t *p_string)
 *
 * @Summary
...
int8_t SIM808_receiveSMS(uint8_t *p_string){
    ...
    //Le copio el comando para recibir los mensajes no leidos
    strcpy( auxBuffer, ATREQ_SMS_REC_NOSAVE );
    
    //Envio el comando
    if( (resultExchange = SIM808_sendATCmd( auxBuffer, true ))==true ){
        //Recibe desde el modulo
        readBytes = SIM808_readATResponse( SIM808_UART_TIMEOUT_ms );       
        //Si hay algo
        if( readBytes>0 ){
            //Busco el comoando AT en el buffer
            if( strstr( c_simRxBuffer, ATRES_SMS_REC )!=0 ){
                //Copio el resultado en p_string
                strcpy(p_string,c_simRxBuffer); 
                retValue = true;
            }
            else{
...
/**
 * @Function
 *  bool SIM808_requestIsValid(uint8_t *p_sentence)
 *
 * @Summary
 *   Función que evalua si el mensaje que pide la cantidad de casos es valido
 *   
 * 
 * @Param:  
 *          p_sentence: Mensaje recibido
 */
bool SIM808_requestIsValid(uint8_t *p_sentence){
    uint8_t offset,i;
    uint8_t p_tel[12];
    ...
    //Utilizo un offset de 8 para llegar al telefono
    offset=8;
    //Obtengo el numero de telefono
    for(i=0;i<11;i++){
        p_tel[i] = p_sentence[offset+i];
    }
    p_tel[i]='\0';
    
    //Obtengo el primer mensaje
    msg = strstr(p_sentence, "\r\n\r\n");
    *msg='\0';
    
    //Obtengo el cuerpo del mensaje
    cuerpo = strstr(p_sentence+2, "\r\n");
    
    //Comparo si el telefono y el comando son los asigandos al dispositivo
    return strcmp(DISP_ComandoGet(),cuerpo+2)==0 && strcmp(DISP_TelefonoGet(),p_tel)==0;
}
...
/**
 * @Function
 *  void SIM808_taskAnswerSMS( void *p_param )
 *
 * @Summary
 *   Tarea que envia por mensaje la cantidad de casos positivos 
 *   Se debe cumplir que el numero desde el que llega la solicitud 
 *   coincida con el que esta configurado en el dispositivo, y que el comando
 *   recibido sea correcto
 */
void SIM808_taskAnswerSMS(void *p_param){
    uint8_t p_str[SIM808_RX_BUF_SIZE];
    uint8_t msg[36];
    uint8_t msg_casos[4];
    while(1){
        if(c_semGSMIsReady != NULL)
        {
            if(xSemaphoreTake( c_semGSMIsReady, portMAX_DELAY ) == pdTRUE)
            {
                memset(p_str,0,sizeof(p_str));
                if(SIM808_receiveSMS(p_str)==1){
                    if(SIM808_requestIsValid(p_str) == true){
                        //Armo el mensaje con la cantidad de casos
                        sprintf(msg_casos,"%d",TEMP_casosPositivosGet());
                        strcpy(msg,"El total de casos positivos es: ");
                        strcat(msg,msg_casos);
                        //Envio mensaje
                        SIM808_sendSMS(DISP_TelefonoGet(),msg);
                    }
                }
